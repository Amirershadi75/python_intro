...
can access every element in a list or every letter in a word one by one using this command:
...
such as printing the value. A while loop is useful when trying to repeat a specific calculation on unknown numbers
